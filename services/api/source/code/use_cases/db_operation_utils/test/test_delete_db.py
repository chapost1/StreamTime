from use_cases.db_operation_utils.concrete import delete_db
import pytest
...
async def test_should_return_none_on_successful_delete():
    ...
    assert await delete_db(deletable=SuccessulDeleter()) is None
...
async def test_should_propagate_exception():
    ...
    try:
        await delete_db(deletable=ExceptionalDeletable())
        # should not reach
        assert 2 == 1
