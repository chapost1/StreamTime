...
Help() {
    ...
    echo "--db_op                  DDL operation on RDS           (init|patch|none)            (default is none)"
    echo
    exit 1
}
...
if is_valid_command "$command"; then
    if is_valid_db_mode "$db_op"; then
        ...
        if [ $command = "destroy" ]; then
            ...
            # lambda layars
            ../lambdas/layers/build_layers.s
            # packages
            echo "no packages to build yet"
        fi

        aws configure set aws_access_key_id $aws_access_key
        aws configure set aws_secret_access_key $aws_secret_ke
        # run command
